...
object SparkGraph {
  ...
  def constructGraph(triples: RDD[Triple]): Graph[Node, Node] = {
    ...
    val indexedMap =
      (rs.map(_._1) union rs.map(_._3)).distinct.zipWithUniqueId()
    ...
    val edges: RDD[Edge[Node]] = tuples
      .join(indexedMap)
      .map({
  ...
  def toDF(graph: Graph[Node, Node]): DataFrame = {
    ...
    val schema = StructType(
      Seq(StructField("subject", StringType, nullable = false),
          StructField("predicate", StringType, nullable = false),
          StructField("object", StringType, nullable = false))
    )
    val rowRDD =
      toRDD(graph).map(t => Row(t.getSubject, t.getPredicate, t.getObject))
    val df = spark.createDataFrame(rowRDD, schema)
    df.createOrReplaceTempView("TRIPLES")
    df
  }

  def toRDD(graph: Graph[Node, Node]): RDD[Triple] = graph.triplets.map {
    case x => Triple.create(x.srcAttr, x.attr, x.dstAttr)
  }
