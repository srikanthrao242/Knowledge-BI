...
class TableCreation extends SparkCoreModule {
  ...
  def createTriplesTable(columns: List[String], data: ObservableBuffer[Array[String]]): TableView[Array[String]] = {
    ...
    columns.indices.foreach(i => {
      ...
      col.text = v
      col.cellValueFactory = { g =>
        new SimpleStringProperty(g.value(i))
      }
  ...
  def createTableForResultSet(
    columns: List[String],
    data: ObservableBuffer[QuerySolution]): TableView[QuerySolution] = {
    ...
    columns.indices.foreach(i => {
      ...
      col.text = v
      col.cellValueFactory = { g =>
        new SimpleStringProperty(g.value.get(v).toString)
      }
      tableView.columns += col
    })
    tableView
  ...
  def saveFile(catalog: String, repository: String, df: RDD[Triple]): Boolean =
    try {
      var path =
        if (catalog.isEmpty) CreateSchemas.getDefaultCatalog
        else CreateSchemas.getWareHousePath + catalog
    ...
    ReadRDF.readNtriples(path).onComplete {
      case Success(v) =>
        ...
        val arr = v
          .map(t => {
            val subj =
              if (t.getSubject.isBlank) t.getSubject.getBlankNodeLabel
              else t.getSubject.getURI
            val pre = t.getPredicate.getURI
            val obj =
              if (t.getObject.isBlank) t.getObject.getBlankNodeLabel
